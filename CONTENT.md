...
Adding new content to the site takes one step: create a single `.mdx` file in the right folder. No code changes needed. The site picks it up automatically.
...
The filename becomes the URL slug. `content/thinking/on-visual-grammar.mdx` → `/thinking/on-visual-grammar`
...
> **Note:** The Thinking index page (`/thinking`) currently uses a data array in
> `src/data/essays.ts` and opens essays as overlays — not MDX slug routes.
> To add an essay that appears on the listing page, add it to that array.
> MDX files in `content/thinking/` are accessible at `/thinking/[slug]` but
> are not currently linked from the listing.

For the data-array essays (`src/data/essays.ts`), each entry looks like:
...
```ts
{
  ...
  tag: "Identity · Craft",
  date: "Jun 2026",
  preview: "One or two sentences shown in the listing.",
  body: `First paragraph.

Second paragraph — double blank line = new paragraph.

Third paragraph.`,
  images: [
    {
      afterParagraph: 1,              // insert after paragraph index (0-based)
...
For MDX-based essay pages (accessible directly via URL), create `content/thinking/your-essay-title.mdx`:
...
date: "2026-06-01"
tag: "Identity · Craft"
preview: "One or two sentences shown in the listing and under the headline."
---
...
`/work/lavou` uses a custom hand-built layout (`src/pages/LavouCaseStudy.tsx`). It is not driven by MDX. Do not create `content/projects/lavou.mdx` expecting it to replace the custom page — the hardcoded route always takes priority.
...
MDX is Markdown with the option to use React components. For most content, standard Markdown is all you need:
